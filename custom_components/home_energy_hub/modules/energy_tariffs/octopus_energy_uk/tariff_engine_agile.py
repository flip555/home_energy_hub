...
async def OctopusEnergyUKTariffEngineAgile(hass, entry):
    ...
    fuel = "Electric"
    async def GET_ELECTRIC_API(region, fuel):
    ...
    async def GET_STANDING_CHARGE_ELECTRIC_API(region, fuel):
        url = f"https://api.octopus.energy/v1/products/AGILE-FLEX-22-11-25/electricity-tariffs/E-1R-AGILE-FLEX-22-11-25-{region}/standing-charges/"
        async with aiohttp.ClientSession() as session, session.get(url) as resp:
            if resp.status == 200:
                data = await resp.json()
                current_time = datetime.utcnow().timestamp()                
    ...
    async def async_update_data():
        ...
        for region in selected_regions:
            ...
            sensors["agile_standing_charge_"+region+fuel] = {
                'state': standing_charge,
                'name': f"Octopus Agile {fuel} - Region {region} - Standing Charge",
                'unique_id': f"Octopus Agile {fuel} - Region {region} - Standing Charge",
                'unit': "p",
                'icon': "",
                'device_class': "",
                'state_class': "",
            ...
            sensors["agile_standing_charge_gbp_"+region+fuel] = {
                'state': standing_charge / 100,
                'name': f"Octopus Agile {fuel} - Region {region} - Standing Charge GBP",
                'unique_id': f"Octopus Agile {fuel} - Region {region} - Standing Charge GBP",
                'unit': "GBP",
                'icon': "",
                'device_class': "",
                'state_class': "",
